...
This is the Gemini CLI-compatible context file. The imported core below is canonical; read and obey it before vault work.
...
For shared-rule changes, edit `10_Meta/agent-core.md` first, then synchronize `AGENTS.md`, `CLAUDE.md`, and `GEMINI.md` in the same change. Keep Gemini-specific guidance only below this heading.

## Gemini-specific notes

Use Gemini CLI’s `@path` import syntax above to load the core. Load only the paths needed for the current task, beginning with the newest useful brief.
